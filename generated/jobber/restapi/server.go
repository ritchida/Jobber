...
//go:generate swagger generate server -t ../.. -A Jobber -f ./swagger.yml

// NewServer creates a new api jobber server but does not configure it
func NewServer(api *operations.JobberAPI) *Server {
	s := new(Server)
	s.api = api
	return s
}
...
// Server for the jobber API
type Server struct {
	Host        string `long:"host" description:"the IP to listen on" default:"localhost" env:"HOST"`
	Port        int    `long:"port" description:"the port to listen on for insecure connections, defaults to a random value" env:"PORT"`
	httpServerL net.Listener

	api          *operations.JobberAPI
	handler      http.Handler
	hasListeners bool
}

// SetAPI configures the server with the specified API. Needs to be called before Serve
func (s *Server) SetAPI(api *operations.JobberAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
...
func (s *Server) Serve() (err error) {
	...
	fmt.Printf("serving jobber at http://%s\n", s.httpServerL.Addr())
	l := s.httpServerL
